...
export class InteractiveSetupWizard {
  ...
  protected async runStep(step: SetupStep): Promise<void> {
    console.log(`\n📋 ${step.name}`);
    ...
    try {
      ...
      console.error(`   ❌ Error during ${step.name}: ${error.message || error}`);
      this.state.errors.push(`${step.name}: ${error.message || error}`);
  ...
  private async checkPlaywrightBrowsers(): Promise<boolean> {
    try {
      ...
      const playwrightCache = path.join(homeDir, process.platform === 'darwin' 
        ? 'Library/Caches/ms-playwright' 
        : '.cache/ms-playwright'
      );
      
      const hasChromium = await fs.access(path.join(playwrightCache, 'chromium-1178'))
        .then(() => true)
        .catch(() => false);
  ...
  protected async waitForInput(prompt: string): Promise<void> {
    await this.rl.question(prompt);
  }

  protected async askYesNo(question: string): Promise<boolean> {
