...
(function() {
  ...
  function setupEffects() {
    ...
    // Create visual cursor for video recordings
    ...
    style.textContent = `
      /* Ensure body has background to hide viewport edges */
      body {
    ...
    window.cinematicControl = {
      ...
      zoomTo: function(scale, centerX, centerY, duration = 800) {
        // Prevent zoom out below 1.0 to avoid showing viewport edges
        scale = Math.max(1.0, scale);
        this.currentZoom = scale;
        
        // Calculate offsets for centering on the target point
        let offsetX = (window.innerWidth / 2 - centerX) * (scale - 1);
        let offsetY = (window.innerHeight / 2 - centerY) * (scale - 1);
        
        // Calculate bounds to prevent showing outside viewport
        if (scale > 1) {
          // Maximum translation to keep content within viewport
          ...
          offsetX = Math.max(-maxTranslateX * scale, Math.min(maxTranslateX * scale, offsetX));
          offsetY = Math.max(-maxTranslateY * scale, Math.min(maxTranslateY * scale, offsetY));
        }
